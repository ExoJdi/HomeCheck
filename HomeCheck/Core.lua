...
function HomeCheck:createCooldownFrame(playerName, spellID, testMode)
    ...
    frame.targetFontString:SetShadowOffset(1, -1)
    frame.targetFontString:SetShadowColor(0, 0, 0, 1)
    frame.targetFontString:SetJustifyH("CENTER")
...
function HomeCheck:repositionFrames(groupIndex)
    ...
    self.groups[groupIndex].CooldownFrames[j]:SetPoint("BOTTOMLEFT", 0, titleBarHeight + (self:getIProp(groupIndex, "iconSize") + self:getIProp(groupIndex, "padding")) * (j - 1))
    end
end
...
function HomeCheck:getGroup(i)
    ...
    frame.titleBar:SetPoint("BOTTOMLEFT", frame, "BOTTOMLEFT", 0, 0)
...
function HomeCheck:setTimerColor(frame)
    ...
    elseif frame.CDLeft <= 0 then
        if self.db.profile.spells[frame.spellID].alwaysShow then
            frame.timerFontString:SetTextColor(0, 0.94, 1, 1)
        end
    else
        frame.timerFontString:SetTextColor(1, 1, 1, 1)
    end
end
...
function HomeCheck:updateTargetPositions(groupIndex)
    for j = 1, #self.groups do
        if j == groupIndex or self.db.profile[j].inherit == groupIndex then
            for k = 1, #self.groups[j].CooldownFrames do
                self:setTargetPosition(self.groups[j].CooldownFrames[k])
            end
        end
    end
end
